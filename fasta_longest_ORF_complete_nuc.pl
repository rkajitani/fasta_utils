...
#		print_seq($aa_seq, 80);
	...
	for $frame (0, 1, 2) {
		$aa_seq = '';
		for ($pos = $frame; $pos + 2 < length($rev_seq); $pos += 3) {
			if (defined $DNA_code{substr($rev_seq, $pos, 3)}) {
				$aa = $DNA_code{substr($rev_seq, $pos, 3)};
			}
			else {
				$aa = 'x';
			}
...
#		print_seq($aa_seq, 80);
		...
		while ($aa_seq =~ /(M[^*]+\*)/g) {
			if ($longest < length $1) {
				...
				$rev_longest_pos = $frame + 3*length($`);
			}
		}
	}

	if ($longest_pos >= 0) {
		$nuc_seq = substr($seq, $longest_pos, 3*$longest);
	}
	else {
		$nuc_seq = substr($rev_seq, $rev_longest_pos, 3*$longest);
	}
		
	printf(">%s_CDS\n", $name);
	print_seq($nuc_seq, 80);
